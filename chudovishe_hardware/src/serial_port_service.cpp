...
using namespace chudovishe_hardware;
...
void SerialPortService::read()
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("SerialPortService"), "Received data: header=0x%X, leftMotorTicks=%d, rightMotorTicks=%d, checksum=0x%X",
...
int SerialPortService::write(const char * message, const int & size)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("SerialPortService"), "Sending %d", size);
